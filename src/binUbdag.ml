...
struct
	...
	let dump man edges =
		let man = H2Table.strdump (Bitv.L.to_hexa_string >> StrTree.of_string) man.man
		and edges = List.map (fst o3b_edge' >> Bitv.L.to_hexa_string >> StrTree.of_string) edges in
		Tree.Node (man::edges)

	let load' hsize = function
		| Tree.Node (man::edges) ->
		(
			let man = H2Table.strload hsize (StrTree.to_string >> Bitv.L.of_hexa_string) man
			and edges = List.map (StrTree.to_string >> Bitv.L.of_hexa_string >> snd o3b_edge') edges in
			({man}, edges)
		)
		| _ -> assert false
