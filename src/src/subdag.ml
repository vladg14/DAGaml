...
struct
	...
	struct
		...
		let do_leaf extra edge leaf =
			((dump_edge edge, Leaf (dump_leaf leaf)):Udag.STree.edge_t)

		let do_node extra edge =
			let merger edge0 edge1 =
				((dump_edge edge, Node (Udag.STree.push extra (Tree.Node [], [edge0; edge1]))):Udag.STree.edge_t)
	...
	struct
		...
		let do_node extra node : (xnode, xnode -> xnode -> xnode) merge =
			...
			MNode(fun son0 son1 -> Node (Udag.STree.push extra (node, [(Tree.Node[], son0); (Tree.Node[], son1)])))
