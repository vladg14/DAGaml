...
struct
	...
	let dump =
		...
		let dump_next_t parcours = function
			| Output idx -> [Tree.Leaf "Output"; STD.int idx]
			| Leaf leaf -> [Tree.Leaf "Leaf"; dump_leaf leaf]
			| NodeRef ident -> [Tree.Leaf "NodeRef"; STD.int ident]
		in
		...
		in fun udag edges ->
			...
			let rec parcours ident = apply ident	(fun ident ->
				(
					...
					push (Tree.Node ((STD.int ident)::(dump_node_t parcours node)))
				)
													)
			in
			let liste_edges =	(
	...
	let load =
		...
		let load_next_t getid = function
			...
			(
				match text with
				| "Output"	-> Output (STL.int b)
				| "Leaf"	-> Leaf (load_leaf b)
				| "NodeRef"	-> NodeRef (getid(STL.int b))
				| _ -> assert false
			)
			| _ -> assert false
		in
