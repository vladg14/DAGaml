...
Iter.iter (fun (block : block) ->
	...
	then
	(Iter.iter (fun set ->
		let edge, _ = assign_edge (Some set) (block, Utils.Node 0) in
		assert(check_edge edge)
	) (gen_assign n));
	if (check_block block true )
	then
	(Iter.iter (fun set ->
		let edge, _ = assign_edge (Some set) (block, Utils.Leaf ()) in
		assert(check_edge edge)
	) (gen_assign n));
	()
) (gen_block_checked n);;
...
Iter.iter (fun blockC ->
	...
	Iter.iter (fun set ->
		let set = Some set in
		let (blockC', identC') as edgeC', set' = assign_edge set (blockC, Utils.Node 1) in
		...
		(
			Iter.iter (fun blockc ->
				...
				(
					...
					assert(check_edge edgec);
					let edgec', (set'' : bool option list option) = assign_edge set' edgec in
					assert(check_edge edgec');
					...
					let edgeCc', (set'3 : bool option list option) = assign_edge set edgeCc in
					assert(check_edge edgeCc');
					assert(edgeC'c' = edgeCc' && set'' = set'3);
