...
let _ = 
	...
	print_string(Cpx2BDump.block test_case0); print_newline();
	...
	print_string(Cpx2BDump.block test_case1); print_newline();
	...
	Iter.iter (fun block ->
		...
		let stream =
			...
			with _ -> print_string(Cpx2BDump.block block); print_newline(); assert(false)
		in
		let block', stream' =
