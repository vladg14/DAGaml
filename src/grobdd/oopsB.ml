...
type edge = CpB.TACX.G0.edge'

let make_const  b n = ((b, MyList.ntimes CpTypes.P n), Utils.Leaf ())

let make_ident man b n = CpB.TACX.G1.push man (TacxTypes.Cons, make_const b n, make_const (not b) n)

let push_pass = CpB.push_pass
...
let (-->) x y : edge = CpBGops.compose x y
let (->>) uniq y : edge = CpBGops.compose ((false, uniq):CpTypes.edge_state) y
...
let copy_fun copy (func : edge) =
	let size = CpB.arity func in
(* f -> [f..., .f.., ..f., ...f] *)
