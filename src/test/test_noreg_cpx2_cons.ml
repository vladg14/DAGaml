...
module T = Cpx2.GroBdd;;

let man = T.newman ();;

let ( *! ) = T.push man;;

let and_man, ( &! ) = Cpx2.AND.newman man;;
let xor_man, ( ^! ) = Cpx2.XOR.newman man;;

let make_bool b = Cpx2.make_const b 0;;
...
Cpx2.GroBdd.dumpfile man (gn |> Iter.enumerate 0 $$ (count "T:1.0 : " 10000 (fun x -> x)) |> Iter.to_list) file;;
print_newline();;
...
gn22 |> Iter.enumerate 0 $$ (count "T:2.0 : " 10000 (fun ((x0, x1), (y0, y1)) ->
	...
	(
		...
		ignore(Cpx2.GroBdd.dumpfile man edges err_file);
		assert false;
	);
	)) |> Iter.iter ignore;;
print_newline();;
...
let peman, peval = Cpx2.PartEval.newman man;;
