...
let parse_cnf_log ?size file_sol =
	...
	| "SATISFIABLE"
	| "SAT" ->
		let liste = input_line file |> StrUtil.split ' ' in
		close_in file;

		let liste = match size with
			| Some size -> fst(MyList.hdtl_nth size liste)
			| None -> liste
		in
		Some (List.map (fun w -> match w.[0] with '-' -> false | _ -> true) liste)
	| "UNSATISFIABLE"
